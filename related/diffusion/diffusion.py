...
from knn import KNN, ANN, KNN_nmslib
...
class Diffusion(object):
    ...
    def __init__(self, features, cache_dir):
        ...
        #self.use_ann = self.N >= 200000
        self.use_ann = self.N >= 10000000
        if self.use_ann:
            self.ann = ANN(self.features, method='cosine')
        self.knn = KNN(self.features, method='cosine')
        #self.knn = KNN_nmslib(self.features, method='cosine')
    ...
    def get_offline_results(self, n_trunc, kd=50):
        ...
        if self.use_ann:
            ...
            print('[offline] 1-1) prepare Laplacian')
        ...
        print('[offline] 1-2) prepare initial state')
